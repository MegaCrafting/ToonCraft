...
public class MobTimer implements Runnable {
	...
	public void run() {
		...
		if(ToonCraft.mobFightMode.size() > 0)
		{
					
			Set<LivingEntity> cogs = ToonCraft.mobFightMode.keySet();
			
			for(LivingEntity cog:cogs)
			{
				
				if(ToonCraft.mobFightMode.get(cog).isEmpty()) {  //mob isn't fighting anyone?  lets cancel.
					ToonCraft.mobFightMode.remove(cog);
					System.out.println("Notice:  a cog was in fight mode but had no opponents..  Removed it from the list");
				} else {
					for(Player p:ToonCraft.mobFightMode.get(cog))  //loop through all the players in the list and do fights.
					{
				
						//Look up the location data (aka fight data) for the player
						LocationData ld = LocationData.findLD(p);
						if(ld != null)
							ld.progressFight();
						else
							System.out.println("No location data found cant progressFight()");
					}
				}
			}
		} 
